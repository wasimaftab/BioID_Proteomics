...
## check for mitochondrial protein
get_mito_protein <- function(proteingroups, Symbol, Uniprot) {
  ## Read a List of Experimentally Determined Mitochondrial Proteins
  Mito_proteins <-
    as.data.frame(
      read.table(
        "Mitochondrial_Proteins/Experimentally_Determined_Mitochondrion_Proteins.txt",
        header = TRUE
      )
    )
  ...
  idx <- which(Symbol %in% Mito_proteins)
  if (length(idx)) {
    proteingroups <- proteingroups[idx,]
    Symbol <- Symbol[idx]
    Uniprot <- Uniprot[idx]
    return(list(proteingroups=proteingroups, Symbol=Symbol, Uniprot=Uniprot))
  } else{
    stop('No mitochondrial proteins in your list, CODE IS TERMINATED')
  }
}
...
display_plotly_figs <-
  function(dat,
    ...
    #
    subDir <-
      paste0("Results_", format(Sys.time(), "%Y%m%d_%H%M%S"))
    mainDir <- getwd()
